...
readonly IN_DIR=$1  # Yohan
readonly BASE_NAME=$2  # Yohan
readonly SPLIT=$3  # Yohan

readonly ADDITIONAL_PSL_OPTIONS='--output out.txt'
readonly ADDITIONAL_LEARN_OPTIONS='--learn -D maxspeudolikelihood.numsamples=10'
...
function runWeightLearning() {
   ...
   java -jar "${JAR_PATH}" --model "${BASE_NAME}.psl" --data "data_${SPLIT}/${BASE_NAME}.data" ${ADDITIONAL_LEARN_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   if [[ "$?" -ne 0 ]]; then
...
function runEvaluation() {
   ...
   java -jar "${JAR_PATH}" --model "${BASE_NAME}-learned.psl" --data "data_${SPLIT}/${BASE_NAME}.data" --output data_${SPLIT}/output ${ADDITIONAL_EVAL_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   if [[ "$?" -ne 0 ]]; then
